    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            # Pass3: Validation / minimal fix (auto)
            # - enable_pass3=True: issuesがある場合のみ実行（デフォルト）
            # - pass3_always=True: issuesが0でも必ず実行（品質優先）
            if enable_pass3:
                from src.llm.validators import detect_issues
                issues = detect_issues(lines, updated_words)
                logger.info("two-pass: pass3 issues detected=%d", len(issues))
                for issue in issues:
                    logger.debug("  - %s", issue.description)

                should_run_pass3 = pass3_always or len(issues) > 0
                if should_run_pass3:
                    if progress_callback:
                    ...
                    logger.info("two-pass: pass3 skipped (0 issues)")
                    record_pass_time(self.run_id, "pass3_skipped", 0.0)
            else:
                logger.info("two-pass: pass3 disabled (enable_pass3=False)")
                record_pass_time(self.run_id, "pass3_disabled", 0.0)
...
# Pass4: re-run only lines that violate length bounds
            ...
            for line in lines:
                if self._needs_pass4(line):
                    ...
                    # まず軽量モデル（pass4_fast_model）で試し、失敗したらフォールバックする
                    ...
                    if repl == [line] and self.pass4_fallback_model and self.pass4_fallback_model != self.pass4_fast_model:
