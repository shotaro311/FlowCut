...
def test_execute_poc_run_save_logs_writes_under_output_dir(tmp_path, monkeypatch):
    ...
    assert (Path("output") / f"{run_id}.srt").exists()
    assert (Path("output/logs/poc_samples") / f"{run_id}.json").exists()
    assert (Path("output/logs/progress") / f"{run_id}.json").exists()

    raw_dir = Path("output/logs/llm_raw")
