...
from src.transcribe.whisper_local_runner import WhisperLocalRunner
from src.transcribe.base import TranscriptionConfig, TranscriptionError


def test_whisper_local_runner_parses_words(monkeypatch, tmp_path: Path):
...
def test_whisper_local_runner_requires_whisper_dependency(monkeypatch, tmp_path: Path):
    audio = tmp_path / "audio.m4a"
    audio.write_bytes(b"dummy")
