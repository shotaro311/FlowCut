...
- **GUI起動**: macOS では `python3 -m src.cli.main gui`（venv 内なら `python -m ...`）でGUI版を起動できます。
...
1. ブランチと依存を整える
   ...
   python -m pip install mlx-whisper mlx pyinstaller
   ```
...
4. PyInstaller で `.app` を生成する（レシピは `FlowCut.spec` を想定）。
   ...
   ```bash
   python -m PyInstaller FlowCut.spec
   ```
