...
* `--save-logs/--no-save-logs` : ログ保存を有効化（SRT出力先に `{入力ファイル名}_{timestamp}/logs/` をまとめて保存）。
...
| パス2 lines が空/不正 | 中 | エラー扱い（ログ保存＋再試行を検討） |
...
| 長時間音声でメモリ不足 | 中 | transcribeは全文処理前提。LLM整形はworkflow2で約5分チャンク分割し、必要に応じて `--resume` で区切り実行する |
