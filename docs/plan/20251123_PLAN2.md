...
| 11-23 00:00 | LLM生応答を `logs/llm_raw/` に保存するデバッグログを追加（Pass1/2/3） | `src/llm/two_pass.py` |
| 11-23 00:00 | JSONパース失敗時にLLM再呼び出しを最大2回試行し、Pass3はフォールバックで継続 | `src/llm/two_pass.py` |
| 11-23 00:00 | Pass3プロンプトに「linesは空配列禁止（1件以上返す）」を追加 | `src/llm/two_pass.py` |
| 11-23 00:00 | Pass3出力を「JSONのみ・説明禁止」に強調し、統一フォーマット例を提示 | `src/llm/two_pass.py` |
| 11-23 00:00 | Pass4（長さ違反行のみ再LLM、ローカル分割なし）を追加 | `src/llm/two_pass.py` |
| 11-23 00:00 | Pass4モデルを `.env` で選択可 (`LLM_PASS4_MODEL`、未指定時は Pass3 と同じ) に対応 | `src/config/settings.py` |
| 11-23 00:00 | 本PLAN新規作成（Pass3必須化タスク） | `docs/plan/20251123_PLAN2.md` |
| 11-24 11:00 | Pass4後のローカル再分割ロジックを削除し、LLM決定の行分割をそのまま採用するように変更 | `src/llm/two_pass.py`, `docs/requirement.md`, `docs/specs/llm_two_pass_workflow.md` |
| 11-24 11:00 | LLM生応答ログを「1 runにつき1ファイル」に集約し、`logs/llm_raw/{音声名}_{日付}_{run_id}.txt` として保存するよう変更 | `src/llm/two_pass.py` |
| 11-24 11:00 | LLM使用量・Pass別処理時間を `logs/metrics/{音声名}_{日付}_{run_id}_metrics.json` に出力（Pass4はトークン数・時間ともに合計値） | `src/llm/usage_metrics.py`, `src/llm/providers/*`, `src/pipeline/poc.py`, `docs/requirement.md` |
| 11-24 11:00 | `temp/poc_samples`, `temp/progress` のファイルをそれぞれ最大5件までに制限し、古い順に自動削除するように変更 | `src/pipeline/poc.py`, `docs/requirement.md` |
...
- **テレメトリ**:
  - Pass3 実行ログに issues 件数・所要時間を記録。
  - LLM呼び出しごとのトークン使用量・処理時間を、Pass1〜3は個別、Pass4は合算値として `logs/metrics/*.json` に記録。
  - 生応答JSONは run 単位で1ファイルに集約して `logs/llm_raw/` に保存。
...
- `src/llm/usage_metrics.py` / `src/llm/pricing.py` / `src/llm/profiles.py` を追加し、LLM使用量・概算コスト・プロファイル管理（provider + Passごとのモデル構成）を一元管理
...
### 追加タスク: LLMログ/コスト可視化 & プロファイル管理
- [x] LLM生応答ログを run 単位で `logs/llm_raw/` に1ファイル集約
- [x] LLM使用量・Pass別処理時間を `logs/metrics/*.json` に出力し、`*_time` 表記で人間が読みやすい形式に統一
- [x] `config/llm_pricing.json` を導入し、プロバイダー/モデルごとの 1M トークン単価から概算コスト（USD）を算出
- [x] `run_total_cost_usd` を含むコスト情報をメトリクスに追加
- [x] `config/llm_profiles.json` + `src/llm/profiles.py` による LLM プロファイル管理（provider + Pass別モデル）を実装し、CLI から `--llm-profile` で指定できるようにする
