...
- [ ] 「モデルプリセット」プルダウンを追加し、`config/llm_profiles.json` に定義した LLMプロファイル一覧を選べるようにする（例: `default`, `low_cost`, `high_quality`）。  
- [ ] 選択されたプリセットをもとに、`PocRunOptions.llm_profile` と `llm_pass1_model`〜`llm_pass4_model` を設定する。  
- [ ] 「詳細設定」エリア（折りたたみ）を追加し、Pass1〜4 のモデル名をテキスト入力で直接上書きできるようにする（上級者向け）。  
