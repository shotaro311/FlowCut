...
- ✅ workflow2 の Pass1 がモデル/コンテキスト都合で失敗した場合、Pass1のみ `LLM_PASS1_MODEL` で再試行（失敗時はエラー表示 + llm_rawに原因を保存）
- ✅ 出力欄に「開く」ボタンを追加し、出力フォルダを直接開けるように改善
- ✅ README にユーザー向けのアプリ説明を追加（`README.md` / `docs/README.md`）
- ✅ Pass5（長行改行）を追加（有効化したときのみ実行）
- ✅ ワークフロー定義を registry 化し、workflow2 のみに統一（workflow1/3 は削除）
...
| 日時 | 変更内容 | 関連ファイル |
|------|---------|-------------|
| 12-18 14:52 | GUIの開始遅延デフォルトを `0.2` 秒に変更 | `src/gui/config.py`, `src/gui/workflow_panel.py`, `docs/README.md`, `docs/plan/20251203_PLAN1.md` |
| 12-18 01:45 | タイムコードずれ対策（行範囲ソート / Pass4の不正範囲検知 / start_delayの負値防止） | `src/llm/two_pass.py`, `tests/test_ranges_to_segments_ordering.py`, `docs/plan/20251203_PLAN1.md` |
| 12-17 23:43 | GUIの「LLMプロバイダー」UIを撤去し、モデル選択でプロバイダー自動判定（Pass5含む）に統一 | `src/gui/workflow_panel.py`, `docs/README.md`, `docs/requirement.md`, `docs/plan/20251203_PLAN1.md` |
| 01-25 2026 | ワークフローを workflow2 のみに統一（workflow1/3 削除） | `src/llm/workflows/registry.py`, `src/gui/workflow_panel.py`, `src/cli/main.py` |
| 01-25 2026 | Whisperランナーを macOS: mlx/whisper-local、Windows: faster/whisper-local に整理 | `src/transcribe/__init__.py`, `src/transcribe/*_runner.py`, `docs/runbook.md` |
...
- **技術スタック**:  
  ...
  - 音声認識: macOS は MLX Whisper / whisper-local、Windows は faster-whisper / whisper-local  
...
- **既知の問題 / 注意点（Windows視点）**:  
  ...
  - MLX Whisper は Metal 依存のため Windows では利用不可 → Windows 版は `faster` / `whisper-local` ランナー前提  
...
- **`src/transcribe/*_runner.py`** :  
  - 役割: Whisper 実行の抽象化。Windows 版では主に `faster` / `whisper-local` ランナーを利用。  
...
- [ ] Whisper ランナー（`src/transcribe/faster_whisper_runner.py`, `src/transcribe/whisper_local_runner.py`）の Windows 対応状況を確認  
...
- [ ] CLI が Windows 上で動作することを確認  
  ...
  - サンプル音声で `run` コマンドを最低1回実行（Whisper ランナーは `faster` を優先）  
