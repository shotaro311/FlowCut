...
※初めて作業に取り掛かる場合は、必ずこのプランドキュメントと別にある要件定義書（`docs/requirements.md`）と runbook（`docs/runbook.md`）も確認してください。  
...
- CLIエントリポイント: `src/cli/main.py`（macOS: `python3 -m src.cli.main gui` / venv内: `python -m src.cli.main gui`）  
...
- ✅ workflow2: 標準のプロンプト/処理に統一（Pass3で範囲変更あり）
- ✅ workflow1/workflow3 を削除し、workflow2 のみに統一（旧 workflow 指定は workflow2 にフォールバック）
...
- ✅ workflow3 の Pass2-4 を統合し、LLM 2回（Pass1 + 統合）で試せるように実験実装（メトリクス表示も追従）
- ✅ workflow3 の「詳細設定」で Pass2-4（統合）のモデル設定のみ表示するように修正
- ✅ GUIの「モデルプリセット」/「LLMプロバイダー」UIを廃止し、詳細設定のモデル選択だけで完結する形に変更（モデル名からプロバイダー自動判定 / Pass1変更で他Passも自動で揃う）
- ✅ Pass5（長行改行）も、モデル選択だけで別プロバイダーを使えるように統一（プロバイダー自動判定）
- ✅ タイムコードずれ対策: 行範囲の順序を正規化し、Pass4 の不正な範囲（from/to）を検知した場合は元行を維持、start_delayで負の字幕長が出ないように補正
- ✅ GUIの「開始遅延」のデフォルトを `0.2` 秒に変更（デフォルトで有効化）
- ✅ workflow2: LLMを約5分単位でチャンク分割して処理（最大10並列）
- ✅ workflow2: 長尺行（1行10秒超）をPass4で再分割するローカル補正を追加（時間情報付きプロンプト）
  - テスト追加で長尺行でもPass4が走ることを確認
- ✅ workflow2: ギャップ埋めの上限を10秒に設定し、長尺固定表示を抑制
- ✅ workflow2: Pass4にStructured Output（JSON Schema）を適用し、分割失敗を減らす
- ✅ workflow2: Pass4プロンプトでfrom/toがワードインデックスであることを明確化（LLMが時間値と混同する問題を修正）
- ✅ workflow2: チャンク統合時のギャップ埋め上限を10秒に設定（漏れていた箇所を対処）
...
| 日時 | 変更内容 | 関連ファイル |
|------|---------|-------------|
| 2026-01-27 | workflow1/workflow3 を削除し workflow2 のみに統一 | `src/llm/workflows/`, `src/pipeline/poc.py`, `src/gui/`, `src/cli/main.py`, `docs/requirements.md` |
| 12-21 16:00 | Pass4プロンプトのfrom/to明確化 + チャンク統合時ギャップ埋め上限設定 | `src/llm/workflows/common.py`, `src/pipeline/poc.py`, `tests/test_two_pass_pass4.py` |
| 12-21 12:20 | workflow2 Pass4にStructured Outputを適用 | `src/llm/two_pass.py`, `docs/requirements.md` |
| 12-20 23:55 | workflow2のギャップ埋め上限を10秒に設定（長尺固定表示の抑制） | `src/pipeline/poc.py`, `docs/requirements.md` |
| 12-20 23:20 | Pass4に長尺行（10秒超）の再分割を追加（時間情報付きプロンプト）+ 長尺行用テストを追加 | `src/llm/two_pass.py`, `src/llm/workflows/common.py`, `tests/test_two_pass_pass4.py`, `docs/requirements.md` |
| 12-20 16:50 | Pass2/Pass3のStructured Outputを有効化 + 欠番検出をPass3に通知（workflow2は欠落でもPass3へ進行） + Pass3に時間情報/長尺分割ルール追加 | `src/llm/two_pass.py`, `src/llm/validators.py`, `src/llm/workflows/workflow1.py`, `tests/test_validators.py`, `tests/test_two_pass_pass2_fallback_split.py`, `docs/requirements.md` |
| 12-20 15:50 | CLI: `--save-logs` を追加（LLM生ログを出力先に保存できるように） | `src/cli/main.py`, `docs/requirements.md` |
| 12-19 23:59 | workflow2: プロンプト/挙動をworkflow1に統一（分割並列のみ維持） | `src/llm/workflows/workflow2.py`, `src/llm/two_pass.py`, `docs/requirements.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 22:54 | workflow2: チャンク並列数を最大10に拡大 | `src/pipeline/poc.py`, `docs/requirements.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 22:34 | workflow2: Structured Outputをworkflow2のみに限定 + GUIのワークフロー説明を更新 | `src/llm/two_pass.py`, `src/gui/workflow_panel.py`, `docs/requirements.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 22:20 | workflow2: 5分チャンク処理 + Pass2のfrom/to再構成（タイムコードずれ混入の抑制） | `src/pipeline/poc.py`, `src/llm/two_pass.py`, `src/llm/chunking.py`, `tests/test_llm_chunking.py`, `tests/test_workflow2_pass2_reconstruct_ranges.py`, `tests/test_two_pass_pass2_fallback_split.py`, `docs/requirements.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 17:58 | workflow2 Pass2 の Structured Output スキーマ適用（Google/OpenAI） | `src/llm/two_pass.py`, `src/llm/providers/google_provider.py`, `src/llm/providers/openai_provider.py`, `docs/requirements.md`, `docs/plan/20251203_PLAN1.md` |
...
- [ ] `docs/requirements.md` の Windows セクションを再確認し、本PLANの内容と齟齬がないか確認  
...
- [ ] `docs/requirements.md` の Windows セクションを更新  
...
- [ ] `docs/requirements.md` に Windows 版配布仕様が反映されている  
