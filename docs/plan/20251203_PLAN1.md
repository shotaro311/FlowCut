...
- ✅ GUIのログ保存（`{subtitle_dir}/logs/`）オプションと、メトリクス簡易サマリ表示を追加  
  - ログ保存ON時: `poc_samples/`, `progress/`, `metrics/`, `llm_raw/` をまとめて保存（デバッグ用）  
  - テスト追加で出力構造を担保
...
| 日時 | 変更内容 | 関連ファイル |
|------|---------|-------------|
| 12-15 19:24 | Pass5（長行改行）の実装とGUI設定追加 | `src/llm/pass5_processor.py`, `src/pipeline/poc.py`, `src/gui/workflow_panel.py`, `src/gui/controller.py`, `src/gui/config.py`, `tests/test_pass5_processor.py` |
| 12-15 18:44 | GUIの処理時間表示を改善（待機/実処理＋Pass1〜4、Pass5は有効時のみ） | `src/gui/controller.py`, `src/gui/workflow_panel.py`, `tests/test_gui_metrics_summary.py` |
| 12-15 18:20 | GUIの「ログ保存」対応（`{subtitle_dir}/logs/`）・メトリクス表示・テスト追加 | `src/pipeline/poc.py`, `src/gui/controller.py`, `src/gui/workflow_panel.py`, `src/llm/two_pass.py`, `tests/test_output_logs_structure.py`, `tests/test_gui_metrics_summary.py` |
...
| 12-04 15:00 | Windows 実機での FlowCut.exe ビルド・動作確認と Whisper assets 同梱手順・ドキュメント更新を反映 | `FlowCut_win.spec`, `docs/requirement.md`, `docs/runbook.md`, `docs/plan/20251203_PLAN1.md` |
