...
- ✅ workflow2: workflow1と同じプロンプト/処理に統一（Pass3で範囲変更あり）
...
| 12-21 12:20 | workflow2 Pass4にStructured Outputを適用 | `src/llm/two_pass.py`, `docs/requirement.md` |
| 12-20 23:55 | workflow2のギャップ埋め上限を10秒に設定（長尺固定表示の抑制） | `src/pipeline/poc.py`, `docs/requirement.md` |
| 12-20 23:20 | Pass4に長尺行（10秒超）の再分割を追加（時間情報付きプロンプト）+ 長尺行用テストを追加 | `src/llm/two_pass.py`, `src/llm/workflows/common.py`, `tests/test_two_pass_pass4.py`, `docs/requirement.md` |
| 12-20 16:50 | Pass2/Pass3のStructured Outputを有効化 + 欠番検出をPass3に通知（workflow2は欠落でもPass3へ進行） + Pass3に時間情報/長尺分割ルール追加 | `src/llm/two_pass.py`, `src/llm/validators.py`, `src/llm/workflows/workflow1.py`, `tests/test_validators.py`, `tests/test_two_pass_pass2_fallback_split.py`, `docs/requirement.md` |
| 12-20 15:50 | CLI: `--save-logs` を追加（LLM生ログを出力先に保存できるように） | `src/cli/main.py`, `docs/requirement.md` |
| 12-19 23:59 | workflow2: プロンプト/挙動をworkflow1に統一（分割並列のみ維持） | `src/llm/workflows/workflow2.py`, `src/llm/two_pass.py`, `docs/requirement.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 22:54 | workflow2: チャンク並列数を最大10に拡大 | `src/pipeline/poc.py`, `docs/requirement.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 22:34 | workflow2: Structured Outputをworkflow2のみに限定 + GUIのワークフロー説明を更新 | `src/llm/two_pass.py`, `src/gui/workflow_panel.py`, `docs/requirement.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 22:20 | workflow2: 5分チャンク処理 + Pass2のfrom/to再構成（タイムコードずれ混入の抑制） | `src/pipeline/poc.py`, `src/llm/two_pass.py`, `src/llm/chunking.py`, `tests/test_llm_chunking.py`, `tests/test_workflow2_pass2_reconstruct_ranges.py`, `tests/test_two_pass_pass2_fallback_split.py`, `docs/requirement.md`, `docs/plan/20251203_PLAN1.md` |
| 12-19 17:58 | workflow2 Pass2 の Structured Output スキーマ適用（Google/OpenAI） | `src/llm/two_pass.py`, `src/llm/providers/google_provider.py`, `src/llm/providers/openai_provider.py`, `docs/requirement.md`, `docs/plan/20251203_PLAN1.md` |
