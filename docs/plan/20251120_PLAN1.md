...
2. `.env` へAPIキー/モデル名を設定し、実装済みの OpenAI/Google/Anthropic クライアントで Dry-run ログを取得する
...
- [📝] Phase 2: LLM整形 + タイムスタンプアライメント実装（40% / `.env`準備 + BlockSplitter実装 + LLM Formatter/3社プロバイダー + タイムスタンプアライナー + SRT生成ユーティリティ + PoC配管）
- [🟡] Phase 3: CLI UX/再開機構/ログ強化（6% / Typer CLIスケルトン + progress仕様ドラフト + `python -m src.cli.main` + `--llm`/`--rewrite`）
...
- [ ] Anthropic APIキー未設定のため3社比較が未完（OpenAI/Googleは `.env` で設定済み）
