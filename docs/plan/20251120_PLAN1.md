...
**現在地**: Phase 1 - 音声認識モデル比較PoC（48%進行 / Runner & CLI PoC完了、評価レポート下書き中）
**次のアクション**:
1. `samples/` に5〜12分の音声（news/podcast）を配置し、各ファイルで `scripts/poc_transcribe.py` を kotoba・mlx・openai の3パターン実行して `temp/poc_samples/*.json` を生成、数値を `reports/poc_whisper_metrics.md` に初回記入する
2. `.env.example` を `.env.local` に複製し APIキー/モデル名を設定、`src/llm/formatter.py` のプロバイダーIF＆テストを作成して Phase 2 Task#2 に着手する
3. 新設した `BlockSplitter` を PoCフロー（`scripts/poc_transcribe.py` / 今後のCLI）へ組み込み、JSONにブロック構造を保存するスタブを追加
...
- [🔄] Phase 1: 音声認識モデル比較PoC（48% / タスク#1-3完了、共有テンプレ整備済み・音源受領＆メトリクス記録待ち）← **今ここ**
- [📝] Phase 2: LLM整形 + タイムスタンプアライメント実装（10% / 詳細プラン + `.env`準備 + BlockSplitter実装済み）
...
- ⏳ 次のタスク: 
  ...
  2. `.env.example` → `.env.local` 複製 + APIキー入力、`src/llm/formatter.py` 雛形生成で Phase 2 Task#2 の準備を完了
  3. `src/blocking/splitter.py` をPoCフローへ組み込み、`temp/poc_samples/*.json` にブロック情報を追記するスタブを追加
