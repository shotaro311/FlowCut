...
**現在地**: Phase 1 - 音声認識モデル比較PoC（60%進行 / Runner & CLI PoC + BlockSplitter/メトリクス/進捗連携済み、サンプル音声で初回CSV生成済み）
**次のアクション**:
1. 5〜12分の正式サンプル音声を `samples/` に追加し、`scripts/poc_transcribe.py` を kotoba・mlx・openai で再実行して `temp/poc_samples/*.json` / `reports/poc_whisper_metrics.csv` を本番値で更新する
2. `.env.local` へAPIキー/モデル名を設定しつつ `src/llm/formatter.py` のレジストリへ OpenAI/Google/Anthropic クライアントを実装、Dry-runできる状態に引き上げる
3. `python -m src.cli.main run --resume ...` の実仕様を詰め、未完ブロックのみ再実行する `--resume` アルゴリズムをLLM整形・SRT出力と結合する
**ブロッカー**: テスト音声データ共有待ち / APIキー（OpenAI, Google, Anthropic）未設定（`.env` 作成を次タスクに組込済み）
**達成済みマイルストーン**:
...
- `src/blocking/splitter.py` と `tests/test_blocking_splitter.py` を追加し、1200文字/30秒 + 末尾重複ロジックを実装
**重要な決定事項**:
...
- [🔄] Phase 1: 音声認識モデル比較PoC（60% / Runner & CLI PoC + BlockSplitter/メトリクス/進捗整備完了、サンプル音声で初回CSV生成済み）← **今ここ**
- [📝] Phase 2: LLM整形 + タイムスタンプアライメント実装（20% / `.env`準備 + BlockSplitter実装 + LLM Formatter/プロバイダー基盤とテスト進行中）
- [🟡] Phase 3: CLI UX/再開機構/ログ強化（5% / Typer CLIスケルトン + progress仕様ドラフト + `python -m src.cli.main` 追加）
...
- [x] `.env.local` を `.env.example` から複製（秘匿値は空、Git対象外に設定）
...
**最終更新**: 2025-11-20 20:05 JST
**進捗率**: 60%

### 直前の作業
- ✅ 完了: `.env.local` を作成し `.gitignore` と合わせて秘匿情報を除外、`src/llm/formatter.py` + `tests/test_llm_formatter.py` で LLMフォーマッター基盤と単体テストを実装（Phase 2 進捗を15%へ更新）
- ✅ 完了: `src/blocking/builders.py` を新設し word-level タイムスタンプから `Sentence` を生成、`scripts/poc_transcribe.py` に BlockSplitter を組み込み JSONへブロック構造を追記（Phase 1 進捗を52%へ更新）
- ✅ 完了: `notebooks/poc_metrics.py` をブロック検証対応へ拡張し、`tests/test_poc_metrics.py` を追加して 1200文字/30秒違反・重複比率を監視できるダッシュボードを準備（Phase 1 進捗を55%へ更新）
...
- ✅ 完了: Phase 2 トラッキングを5%に引き上げ、`.env` 整備/ブロック分割器着手を次工程へ組込み
- ✅ 完了: サンプル共有手順テンプレ（リンク/sha256/免責文）を `samples/README.md` に追記し、共有依頼のテンプレートを確定
- ✅ 完了: `src/blocking/README.md` + `src/blocking/splitter.py` + `tests/test_blocking_splitter.py` を追加し、ブロック分割ロジックと単体テストを実装
...
- ⏳ 次のタスク: 
  ...
  2. `.env.local` へAPIキー/モデル名を設定して LLMプロバイダークラス（OpenAI/Google/Anthropic）を登録し、Dry-runログを取得
  3. 生成された `progress_*.json` をCLI `--resume` フローに接続する仕様ドラフトを作成し、段階的なブロック再開アルゴリズムを決定
...
| 11-20 18:55 | ブロックメトリクス集計を notebooks/poc_metrics.py に実装し、テストを追加して進捗55%化 | `notebooks/poc_metrics.py`, `tests/test_poc_metrics.py`, `docs/plan/20251120_PLAN1.md` |
| 11-20 18:35 | BlockSplitterをPoC出力に連携する builders/sentence 化と JSON拡張、Phase 1 進捗52%化 | `src/blocking/builders.py`, `scripts/poc_transcribe.py`, `tests/test_sentence_builder.py`, `docs/plan/20251120_PLAN1.md` |
| 11-20 18:05 | `.env.local` 整備と LLMフォーマッター基盤 + 単体テスト追加、Phase 2 進捗15%化 | `.gitignore`, `.env.local`, `src/llm/*`, `tests/test_llm_formatter.py`, `docs/plan/20251120_PLAN1.md` |
| 11-20 17:40 | BlockSplitter実装＆テスト追加、フェーズ進捗48%化 | `src/blocking/*`, `tests/test_blocking_splitter.py`, `docs/plan/20251120_PLAN1.md` |
...
| `src/blocking/builders.py` | 🆕 初期実装 | WordTimestamp → Sentence 変換（BlockSplitter入力） | ブロック分割 | Phase 2 |
| `scripts/poc_transcribe.py` | 🆕 更新 | PoCフローを `src/pipeline/poc.py` へ委譲し共通オプション化 | PoCフロー | Phase 1 |
| `src/pipeline/poc.py` | 🆕 初期実装 | ランナー実行・ブロック保存・progress更新の共通ロジック | PoCフロー | Phase 1 |
| `tests/test_sentence_builder.py` | 🆕 初期実装 | buildersユーティリティの単体テスト | テスト | Phase 2 |
| `notebooks/poc_metrics.py` | 🆕 更新 | ブロック検証/違反カウント/CSV拡張 | メトリクス | Phase 1 |
...
| `reports/poc_whisper_metrics.csv` | 🆕 初期値記入 | シミュレーションサンプルのブロック指標 | レポート | Phase 1 |
...
- `segment_latency_ms`（1ブロックあたりの平均推論時間）
...
### ゴール
- LLMプロンプト/ブロック分割/17文字検証を自動化し、整形テキストに `[WORD: xxx]` を付与できる状態にする。
...
- `src/blocking/splitter.py`（仮）: 1200文字/30秒・末尾1-2文重複のブロック分割
...
| # | タスク | 詳細 | 入力/依存 | 完了判定 |
|---|--------|------|-----------|----------|
| 1 | ブロック分割器実装 | 1200文字 or 30秒超で分割、末尾1-2文を次ブロックへ複製 | Phase1のword timestamps | 単体テストで閾値・重複を検証 |
| 2 | プロンプト/プロバイダーIF | OpenAI/Google/Anthropicを `.env` からロードし `--llm` で切替 | 要件定義2章 | 3社それぞれでDry-runし成功ログを保存 |
| 3 | `[WORD: ]` パーサ | LLM出力をJSON化し17文字検証→超過時に自動再分割 | タスク1/2 | `logs/resplit.log` に記録されるテストケースを整備 |
| 4 | タイムスタンプアライナー | アンカー一致→RapidFuzz(90→85→80%)→補間 | タスク1/3 | `reports/alignment_scenarios.md` に10ケース記録 |
| 5 | 結合テスト | 5分+12分サンプルでWER, RTF, 警告件数を収集 | タスク1-4 | `reports/poc_whisper_metrics.md` にLLM列追加 |
...
### テスト観点
- ブロック長境界（1199/1200文字、29.9/30秒）での挙動
...
| 3 | 再開機構 | ブロックごとに中間結果を `temp/progress_*.json` へ書き出し、再開時にstate再構築 | 疑似失敗で `--resume` が同じ行から続行 |
...
- デフォルトモデル: `kotoba`、デフォルトLLM: `openai`
...
| 3 | D&D/進捗UI | 音声ファイルのドラッグ受け付け、進捗バーをCLIのブロック進捗に連動 |
