...
**次のアクション**:
1. 5〜12分の正式サンプル音声を `samples/` に追加し、3モデルで実行して `temp/poc_samples/*.json` / `reports/poc_whisper_metrics.csv` を本番値に更新（現状 OpenAI/MLX 実走済み、BlockSplitter 無効）
2. LLM整形 + アラインの実データ検証を行い、`--align-thresholds` / `--align-gap` などのデフォルトをチューニング
3. `python -m src.cli.main run --resume ...` のE2Eスモークを追加し、長尺で途中再開→SRT出力まで確認
...
- `src/blocking/splitter.py` と `tests/test_blocking_splitter.py` を追加（必要に応じて `--block-splitter` で有効化/無効化を切替）
**重要な決定事項**:
- 音声認識モデルは kotoba-mlx / mlx-large-v3 / openai-whisper を同一CLIで切り替えて比較
- LLM整形は OpenAI/Google/Anthropic を `.env` + `--llm` オプションでホットスワップ（デフォルトプロバイダーは Google）
- Blocking は当面無効（全文をLLMへ渡して意味的改行をLLM側で実施）。分割ロジック再調整は後続タスク
...
- [📝] Phase 2: LLM整形 + タイムスタンプアライメント実装（55% / リトライ・調整オプションと SRT 出力を実装済み、実データで閾値調整と精度評価待ち）
...
**最終更新**: 2025-11-21 22:00 JST
**進捗率**: 86%

### 直前の作業
- ✅ MLX Whisper large-v3 をローカル実行（BlockSplitter無効）し、5分サンプルを約33秒で完了 (`--no-block-splitter`)
...
- [ ] BlockSplitter は当面無効運用。再チューニング（句読点なし長文の強制分割など）は後続タスクで対応

### 最近の変更履歴（直近5件）
| 日時 | 変更内容 | 関連ファイル |
|------|---------|-------------|
| 11-21 21:23 | BlockSplitter無効オプション追加、MLX large-v3ローカル実走完了（約33s） | `src/cli/main.py`, `src/pipeline/poc.py`, `temp/poc_samples/sample_audio_mlx_20251121T212357.json` |
...
- **ブロック処理**: 当面無効化（全文をLLM整形に渡す）。必要時のみ `--block-splitter` で有効にする方針
...
| `src/blocking/builders.py` | 🆕 初期実装 | WordTimestamp → Sentence 変換（BlockSplitter入力、現状オプション運用） | ブロック分割 | Phase 2 |
...
- **`BlockSplitter`（オプション）**: デフォルト無効。必要時のみ `--block-splitter` で有効化
...
- [ ] （延期）BlockSplitter再設計。現状は全文をLLMへ渡し、分割はLLM整形に委ねる
...
## Phase 2 詳細プラン（LLM整形 + タイムスタンプ）※Blocking無効運用

### ゴール
- LLMプロンプト/17文字検証を自動化し、整形テキストに `[WORD: xxx]` を付与できる状態にする（ブロック分割は当面行わない）。
...
| # | タスク | 詳細 | 入力/依存 | 完了判定 |
|---|--------|------|-----------|----------|
| 1 | （延期）BlockSplitter再設計 | 現行は無効運用。句読点希薄・安全弁のみ後続で検討 | Phase1のword timestamps | 後続タスクとしてドキュメント化 |
...
- 途中失敗時にも `temp/progress_*.json` を用いて `--resume` で再処理できる堅牢性を確保（ブロック分割なしでも進捗保存）。
...
- デフォルトモデル: 未指定時は全ランナー実行、デフォルトLLM: `google`
...
- **動作確認済み機能**: Whisper (OpenAI/MLX) 実走、LLM整形+SRT（OpenAI）、BlockSplitter無効オプション
- **既知の問題**: BlockSplitter再設計は後続。長尺音声の分割ロジックは当面無効運用で回避中
