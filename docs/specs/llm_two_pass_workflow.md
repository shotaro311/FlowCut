...
5. ローカルでSRT生成: 行開始=最初の単語start、行終了=最後の単語end。行テキストの長さは LLM（Pass2/Pass3/Pass4）が決めたものをそのまま採用し、巻き戻りのみクランプで補正する。
...
- 行開始: 行の最初の単語 start
- 行終了: 行の最後の単語 end
- 行テキストの長さは LLM 側（Pass2/Pass3/Pass4）の責務とし、ここでは変更しない（長さによる再分割は行わない）
...
- 出力が空/不正の場合は元行をそのまま維持する。Pass4 後の `lines` は、そのまま SRT 生成に使用し、ローカル強制再分割は行わない。
