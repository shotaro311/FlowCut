...
4. パス2（LLM分割）: パス1後の単語列を入力し、17文字幅を守る行区切りをインデックス範囲で返す。
5. ローカルでSRT生成: 行開始=最初の単語start、行終了=最後の単語end。幅超過があればローカルで再分割し時間を区間内で線形補間。最後に音声尺でクランプ。
