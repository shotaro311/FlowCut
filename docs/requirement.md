...
※ 基本運用では **`GOOGLE_API_KEY` があれば十分** で、OpenAI の LLM を使う場合にのみ `OPENAI_API_KEY` が必要（それ以外のケースでは未設定でもよい）。  
...
* macOS は `mlx` / `whisper-local`、Windows は `faster` / `whisper-local` を使用し、音声からテキストデータを作成。
...
* **ワークフロー定義**
  * `src/llm/workflows/workflow2.py` にプロンプトと挙動を定義する。
  * GUI/CLI は `workflow2` 固定で実行する。
  * 不明な workflow が指定された場合は `workflow2` にフォールバックする。
* **プロンプトの役割（two-pass）:**
...
* `--models mlx,whisper-local` : 使用するランナーをカンマ区切り指定。未指定なら **プラットフォーム別デフォルト**（macOS: `mlx` / Windows: `faster`）を実行。
...
```bash
# デフォルトランナーで文字起こし（整形なし）
python -m src.cli.main run samples/sample_audio.m4a

# macOS: mlx と whisper-local を実行し、Geminiで整形＋SRT出力
python -m src.cli.main run samples/sample_audio.m4a \
  --models mlx,whisper-local \
  --llm google
...
* 2モデル（macOS: mlx / whisper-local、Windows: faster / whisper-local）実装と比較
